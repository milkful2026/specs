...
| **Repos touched** | `milkful2026/services` (MA-130, MA-131, MA-132), `milkful2026/milkful-app` (MA-133) |
...
- Confirmed by reading `services/adapters/wallet_limits_client.py` directly: **this codebase's internal service-to-service calls are plain unauthenticated HTTP** (`requests` + `shared.adapters.retry.call_with_retry`), not real SigV4/mTLS, despite MA-130 §5's NFR table describing "Internal-only network exposure ... security-group-restricted." Every internal adapter in this plan (Order → User, Order → Pricing, Order → Wallet) follows that same established pattern — a real signer is a swap-in-one-adapter change for later, per that file's own doc comment. **Unlike `wallet_limits_client.py`, none of this plan's new adapters use its "cache + fall back to a safe default on failure" behavior** — MA-132.md is explicit that a User/Pricing/Wallet call failure must fail the order materialization closed (message left unacked), never guess a state, a price, or a debit outcome.
...
| `RegistrationBloc` access from `ProductConfigScreen`/`SubscriptionsScreen` | **Already satisfied** | Provided app-wide in `main.dart`; `context.read<RegistrationBloc>()` works from any screen. |
...
6. **MA-133 Flutter My Subscriptions & Lifecycle Screens** — **may start in parallel with step 1** against `FakeSubscriptionRepository`, same precedent as MA-125 relative to MA-126/MA-127; the real `DioSubscriptionRepository` is validated against the live services once steps 2–5 are done. No feature flag (unlike MA-125's `WALLET_ENABLED`) — ships live once merged, per MA-133 §4 FR-1's own decision.
...
**Files to modify:**
- `services/wallet/src/domain/exceptions.py` — add `OrderUserMismatchError(WalletError)` (`error_code="ORDER_USER_MISMATCH"`, `http_status=400`) and `InvalidAmountError(WalletError)` (`error_code="INVALID_AMOUNT"`, `http_status=400`). **`WALLET_NOT_ACTIVE` is deliberately not a new exception class** — per MA-130.md §4 FR-1 (review-fixed), it's a normal 200 response value on `DebitOutcome`, not an HTTP error, so it never goes through the existing `@app.exception_handler(WalletError)` path.
...
- `services/wallet/src/adapters/wallet_repository.py` — add `debit_for_order(*, user_id, order_id, amount_paise, correlation_id, outbox_payload_builders)`, one `engine.begin()` transaction:
  ...
  2. No wallet row, or `status != ACTIVE` → return `WALLET_NOT_ACTIVE` (no write). Checked **before** the ledger-replay step, since a ledger comparison is meaningless without a locked wallet.
...
- `services/wallet/src/handlers/internal_handlers.py` — add `POST /wallet/internal/debit` (body `{userId, orderId, amountPaise, correlationId?}`, all required except `correlationId`) and `GET /wallet/internal/balance?userId=`, same thin/SigV4-gated-in-theory convention as the existing `GET /wallet/internal/limits` in this file.
...
- `services/subscription/src/domain/subscription_service.py` — `SubscriptionService`:
  - **`is_due(subscription, target_date) -> bool`** — the reusable due-computation function MA-131.md's review-fix requires be callable for an arbitrary `target_date`, not just literally "tomorrow": `target_date < subscription.start_date` → `False` unconditionally (the explicit guard the review added, applying uniformly before any type-specific check — **not** an incidental property of `ALTERNATE_DAYS`'s parity formula, which the pre-fix spec relied on and which doesn't actually hold for a `target_date` far enough before `start_date`); else `DAILY` → `True`; `ALTERNATE_DAYS` → `(target_date - start_date).days % 2 == 0`; `WEEKLY`/`CUSTOM_DAYS` → `target_date`'s ISO weekday in `schedule.days_of_week`. Also excludes a `target_date` inside `[pause_from, pause_until]` (or from `pause_from` onward if `pause_until` is absent) and a `target_date` matching a `subscription_skips` row.
  ...
  - `run_daily(now) -> list[str]` — FR-8: for every `ACTIVE` subscription, first applies any `pending_edit` whose `effective_from <= tomorrow`, then calls `is_due(subscription, tomorrow)`; for each due one, emits `SubscriptionOrderDue{deliveryDate: tomorrow}` unless `(subscription_id, tomorrow)` is already in `subscription_run_log` (Scheduler-retry idempotency). One bad subscription's exception is logged and does **not** abort the run (MA-131 §5 NFR: "isolate failures").
- `services/subscription/src/adapters/interfaces.py` — `SubscriptionRepositoryPort`, `CatalogClientPort`.
- `services/subscription/src/adapters/subscription_repository.py` — Aurora (psycopg): `insert_if_absent` (idempotency key), `get_by_id`, `list_by_user`, `update_status`/`update_pause`/`update_pending_edit`, `insert_skip`, `has_run_logged(subscription_id, date)`, `insert_run_log`, `list_active` (for the Daily Run's batch query — MA-131 §5 NFR: "batched due-date computation, not one query per subscription"), plus the shared outbox-insert-in-the-same-tx pattern `credit_recharge`/`debit_for_order` both already establish.
...
3. Implement `SubscriptionRepository` (psycopg), including `list_active` batched for the Daily Run.
...
- `services/order/src/domain/order_service.py` — `OrderService`:
  - `materialize(subscription_id, user_id, product_id, quantity, delivery_date, slot_id, correlation_id) -> None` — FR-1/FR-2, the consumer's core logic:
    1. `(subscription_id, delivery_date)` already has an `Order` row → no-op (idempotency, DB-level UNIQUE backstop).
    2. Resolve `deliveryState` via `user_client.get_delivery_address_state(user_id)` (User Service's `GET /v1/internal/users/address-state?cognitoSub={userId}` — **the same internal endpoint Cart Service already calls for the identical need**, confirmed reading `cart/src/adapters/user_client_adapter.py`). No profile / unavailable → `PAYMENT_FAILED`-shaped materialization with `reason: DELIVERY_ADDRESS_UNKNOWN` (message stays unacked at the handler level so this whole attempt retries — see the consumer below).
    ...
    4. Insert the `Order` row `CREATED` + an outbox row reserved for the eventual `OrderConfirmed`/`OrderPaymentFailed` — one transaction (§6: "insert-then-call-then-update... `(subscription_id, delivery_date)` UNIQUE at insert time is what makes re-entry after a crash... safe").
    5. Call `wallet_client.debit(user_id, order_id, amount_paise, correlation_id)` (Wallet Service's new `POST /wallet/internal/debit`, Step 2) **outside** the DB transaction from step 4 (an external HTTP call inside a held DB transaction is the exact anti-pattern MA-132 §6 calls out as avoided elsewhere in this codebase) — `DEBITED` → `set_status(CONFIRMED, confirmed_at=now)` + enqueue `OrderConfirmed`; `INSUFFICIENT_BALANCE`/`WALLET_NOT_ACTIVE` → `set_status(PAYMENT_FAILED, failure_reason)` + enqueue `OrderPaymentFailed`; a transport failure/timeout → leave `CREATED`, propagate the exception so the consumer (below) does **not** ack the message (safe to redeliver because of Wallet's own `orderId` idempotency, Step 2).
...
- `services/order/src/adapters/user_client_adapter.py` — `GET {ORDER_USER_INTERNAL_BASE_URL}/v1/internal/users/address-state?cognitoSub={userId}`, `shared.adapters.retry.call_with_retry`, no fallback (fails closed, matching `materialize` step 2 above) — mirrors `cart/src/adapters/user_client_adapter.py`.
...
- `services/order/src/adapters/wallet_client_adapter.py` — `POST {ORDER_WALLET_INTERNAL_BASE_URL}/wallet/internal/debit`, branches on the response's `status` field (`DEBITED`/`INSUFFICIENT_BALANCE`/`WALLET_NOT_ACTIVE` all 200s per Step 2's contract-shape fix — this adapter must **not** treat any of the three as an HTTP error).
...
- `services/order/infra/` — CDK stack (`order_stack.py`) mirroring `inventory_stack.py`'s Fargate pattern: Fargate service, Aurora `orders`, **owns `order-events-q` + DLQ** (this service's own queue, unlike Wallet consuming a queue Payment's stack targets — Order Service creates and owns the queue Subscription Service's EventBridge rule targets), IAM (`sqs:*` on its queue, `events:PutEvents`, `rds` connect, no outbound SigV4 needed per §1's unauthenticated-internal-call finding).
...
2. `0001_orders.sql`.
3. `user_client_adapter.py`, `pricing_client_adapter.py`, `wallet_client_adapter.py` — all fail-closed, no fallback; `wallet_client_adapter.py`'s three-way 200-status branch specifically (not an error-status branch).
...
**Files to modify:**
- `services/local-dev/docker-compose.yml` — add `subscription` (build `../subscription`, port `8008:8008`), `order` (build `../order`, port `8009:8009`, plus an `order-events-consumer` thread already inside `main.py` — no separate container needed, mirroring `wallet`'s own single-deployable-does-both convention), **and** the still-missing `cart` (build `../cart`), `payment` (build `../payment`, port `8007:8007`), `wallet` (build `../wallet`, port `8006:8006`) entries carried over from MA-24 (§1) — all `depends_on: [bootstrap]`. Outbox-loop and reconcile/invariant-check containers for every service that needs one (`payment-reconcile`, `wallet-invariant-check`, `order-outbox`, `subscription-outbox`, ...), matching each service's own established pattern.
...
3. `docker-compose.yml` — all five outstanding services (`cart`, `payment`, `wallet`, `subscription`, `order`) + their outbox/reconcile loops.
...
- `lib/features/cart/bloc/product_config_bloc.dart` — `_onStarted`: for a subscription-eligible product, also fetch delivery slots (`RegistrationRepository.getDeliverySlots(zoneId)`) and default-select the first `available` one; `_onSlotSelected` handler; `_onAddToCartRequested`: when `state.frequency.isSubscription`, call `SubscriptionRepository.create(productId, quantity, schedule, startDate, slotId, idempotencyKey)` (MA-131 FR-1) **instead of** `CartRepository.addItem(...)` — a one-time frequency confirm is unchanged.
...
5. `ProductConfigState`/`Event`/`Bloc` — the slot picker addition + the `slotId`-aware `create()` call. **`ProductConfigBloc._onAddToCartRequested` is a real, tested behavior change** — extend `product_config_bloc_test.dart`, don't replace it (MA-133 §11 Risk).
...
- Unit (`product_config_bloc_test.dart` additions): the slot chip row renders once `getDeliverySlots` resolves, defaults to the first available slot; `SlotSelected` updates state; `canConfirm` is `false` with no slot selected (subscription frequency only — one-time is unaffected); `AddToCartRequested` for a subscription frequency calls `SubscriptionRepository.create` (not `CartRepository.addItem`) with the selected `slotId`; a one-time-frequency confirm still calls `CartRepository.addItem` exactly as before, `SubscriptionRepository.create` never called.
- Widget (`product_config_screen_test.dart` additions): Subscribe Now is disabled with no delivery slots returned; creating a DAILY subscription shows the slot chips, submits the selected one.
...
- **EventBridge topology** — Subscription Service's stack owns the new `SubscriptionOrderDue → order-events-q` rule; Order Service's stack owns `order-events-q` + DLQ itself (unlike Wallet, which consumes a queue *Payment's* stack targets — here Order both owns and is targeted, since nothing else currently produces to this queue). Local-dev `bootstrap.py` mirrors this.
...
- `chore(MA-25): local-dev wiring for subscription (:8008), order (:8009), and the still-missing cart/payment/wallet compose entries` — Step 5
...
- `feat(MA-25): ProductConfigScreen — delivery-slot picker, subscription create re-pointed to SubscriptionRepository` — Step 6.5
...
| **Order Service's synchronous critical path now chains three internal HTTP calls** (User → Pricing → Wallet) before an order is confirmed | Each has its own retry+backoff (`shared.adapters.retry`); MA-132 §5's p99 target (700ms, this plan's revised number) assumes all three are in-VPC/same-region internal calls, not real network hops. If this proves too slow in practice, User's address-state result is cacheable per-user (not built here — no evidence of need yet, same "build it when you have evidence" restraint MA-132 §11 already applies to its own reconciliation-sweep decision). |
