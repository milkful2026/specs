...
- **Pending-key scope:** `checkout.pendingKey.{sub}` (Cognito `sub` from the stored token) per MA-137 FR-9, updated on `spec/MA-34` so the key never depends on a network read.
